"""Re-summarize corpus-covered transactional items under the v3.2 prompt.
...
summarized under pre-v3.2 prompts, where every production-eligible attachment's
...
API path (v3.2 prompt, token-aware chunking), then ingested via the production
...
Usage:
    uv run scripts/backfill_v32_summaries.py --since-days 30 [--limit N] [--dry-run]

State persists to data/backfill_v32_state.jsonl; re-running resumes open jobs,
...
PROMPT_VERSION = "v3.2"
logger = get_logger(__name__).bind(component="backfill_v32")

# Never reuse a state ledger across prompt versions. In particular, a v3.1
# ledger can contain both IDs already ingested under v3.1 and still-open v3.1
# Gemini jobs. Loading either while running v3.2 would skip required work or
# ingest an old result while labeling it v3.2.
STATE_PATH = Path(
    f"/opt/engagic/data/backfill_{PROMPT_VERSION.replace('.', '')}_state.jsonl"
)
WAVE_SIZE = 200
...
WHERE i.summary IS NOT NULL
  AND i.prompts_version IS DISTINCT FROM '{PROMPT_VERSION}'
...
def load_state() -> tuple[set, set, list, dict]:
    ...
    with open(STATE_PATH) as f:
        for line in f:
            rec = json.loads(line)
            if rec.get("prompts_version") != PROMPT_VERSION:
                logger.warning(
                    "ignoring state record for another prompt version",
                    record_version=rec.get("prompts_version"),
...
def append_state(rec: dict) -> None:
    rec = {**rec, "prompts_version": PROMPT_VERSION}
...
async def main() -> None:
    ...
    assert summarizer.prompts_version == PROMPT_VERSION, (
        f"backfill requires the {PROMPT_VERSION} prompt"
    )
