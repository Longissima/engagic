<script lang="ts">
	...
	const totalVotes = $derived(data.cities.reduce((sum, c) => sum + c.vote_count, 0));
...
<SeoHead
	title="Committees by City - engagic"
	description="View committee coverage across cities tracked by Engagic"
	url="https://engagic.org/committees"
/>

<article class="page-container">
	<header class="page-header">
		<h1 class="primary-heading">Committees by City</h1>
		<p class="section-desc">Cities with legislative committees tracked, sorted by population. Click a city to view its committee list.</p>
	</header>

	<div class="stats-summary">
		<div class="summary-item">
			<span class="summary-count">{data.totals.cities_with_committees}</span>
			<span class="summary-label">Cities</span>
		</div>
		<div class="summary-item">
			<span class="summary-count">{formatNumber(data.totals.total_committees)}</span>
			<span class="summary-label">Committees</span>
		</div>
		<div class="summary-item">
			<span class="summary-count">{formatNumber(data.totals.total_assignments)}</span>
		...
		<table class="jurisdiction-table">
			<thead>
				<tr>
					...
					<th class="col-count">Assignments</th>
			...
			<tbody>
				{#each data.cities as city}
					<tr>
						<td class="col-city">
							<a href="/{city.banana}/committees" class="jurisdiction-link">
								{city.city_name}, {city.state}
							</a>
						</td>
						<td class="col-count">{city.committee_count.toLocaleString()}</td>
						<td class="col-count">{city.assignment_count.toLocaleString()}</td>
						<td class="col-votes">
